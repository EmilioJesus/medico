...
class citaspaciente extends React.Component {
    ...
    citas() {
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                db.collection("Usuarios/" + uid + "/Citas").orderBy("timestamp","asc").onSnapshot((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        ...
                        var id = doc.id;
                        var hoy = new Date();
                        var fechahoy = Date.parse(hoy.getUTCFullYear() + '/' + (hoy.getMonth() + 1) + '/' + hoy.getDate());
                        if (fechahoy > cita) {
                            db.collection("Usuarios").doc(uid).collection('Citasrealizadas').doc(id).set({
                                iddoctor: id,
                                Imagenperfil: imagen,
                                Motivocita: motivo,
                                ...
                                .then((docRef) => {
                                    ...
                                    db.collection("Usuarios").doc(uid).collection('Citas').doc(id).delete().then(() => {
                                        console.log("Document successfully deleted!");
                                        if(tipocita=="Presencial") 
                                        {
                                            db.collection("Usuarios/"+iddoctor+"/HorariosPresencial").doc(dia).collection(dia+"1").doc(indice).update({
                                                hora:horario
                                                });
                                        }else
                                        {
                                            db.collection("Usuarios/"+iddoctor+"/HorariosLinea").doc(dia).collection(dia+"1").doc(indice).update({
                                                hora:horario
                                                });
                                        }
                                    }).catch((error) => {
   ...
   cancelar() {
        ...
        if (id != null) {
            firebase.auth().onAuthStateChanged((user) => {
                var uid = user.uid;
                db.collection("Usuarios/" + uid + "/Citas").doc(id).onSnapshot((doc) => {
                        console.log(`${doc.id} => ${doc.data()}`);
                        if(doc.exists)
                        {
                        var imagen = doc.data().Imagenperfil;
                        ...
                        db.collection("Usuarios").doc(uid).collection('Citascanceladas').add({
                            Imagenperfil: imagen,
                            Motivocita: motivo,
                            Primeracita: primeracita,
                            Nombrecita: nombre,
                            ...
                            db.collection("Usuarios/" + iddoctor + "/Citas").doc(id).onSnapshot((doc) => {
                                if(doc.exists)
                                {
                                    var imagen = doc.data().Imagenperfil;
                                    var nombre = doc.data().Nombrecita;
                                    ...
                                    var dia = doc.data().Diacita;
                                    var indice = doc.data().Indice;
                               db.collection("Usuarios").doc(iddoctor).collection('Citascanceladas').doc(docRef.id).set({
                                Imagenperfil: imagen,
                                Motivocita: motivo,
                                Primeracita: primeracita,
                                Nombrecita: nombre,
                                ...
                                db.collection("Usuarios").doc(uid).collection('Citas').doc(id).delete().then(() => {
                                    console.log("Document successfully deleted!");
                                    db.collection("Usuarios").doc(iddoctor).collection('Citas').doc(id).delete().then(() => {
                                        console.log("Document successfully deleted!");
                                        if(tipocita=="Presencial") 
                                        {
                                            db.collection("Usuarios/"+iddoctor+"/HorariosPresencial").doc(dia).collection(dia+"1").doc(indice).update({
    ...
    citascanceladas()
    {
        ...
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                db.collection("Usuarios/" + uid + "/Citascanceladas").orderBy("timestamp","asc").onSnapshot((querySnapshot) => {
                    querySnapshot.forEach((doc) => {   
                        var imagen = doc.data().Imagenperfil;
                        var nombre = doc.data().Nombrecita;
                        var apellidop = doc.data().ApellidoPcita;
    ...
    citasrealizadas()
    {
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                db.collection("Usuarios/" + uid + "/Citasrealizadas").orderBy("timestamp","asc").onSnapshot((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        var imagen = doc.data().Imagenperfil;
                        var nombre = doc.data().Nombrecita;
                        var apellidop = doc.data().ApellidoPcita;
    ...
    render() {
        return (
            <div onLoad={this.citas}>
                ...
                <h2>Citas</h2>
                <div id="resultadocitas" style={{ overflow: 'auto', }}>
                </div>
