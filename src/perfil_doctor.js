...
class perfil_doctor extends Component {
      ...
      if (user) {
        ...
        db.collection("Usuarios").doc(uid)
          .onSnapshot((doc) => {
            ...
            var terminoinscrip = new Date(terminoinscripcion).toLocaleDateString("en-US");
            ...
            document.getElementById("terminoinscripcion").innerText = terminoinscrip;
  ...
  agregar_horario()
  {
    ...
    if(horainicio!="" && horatermino!="")
    {
      if(tipo=="En linea")
      {
       db.collection("Usuarios").doc(uid).collection("HorariosLinea").doc(dia).collection(dia+"1").add({
         hora:horacita,
         timestamp: firebase.firestore.FieldValue.serverTimestamp()
      }).then((docRef) => {
        db.collection("Usuarios").doc(uid).collection("HorariosLineaDoctor").doc(dia).collection(dia+"1").doc(docRef.id).set({
         hora:horacita,
         timestamp: firebase.firestore.FieldValue.serverTimestamp()
      }).then((docRef) => {
      ...
      {
        db.collection("Usuarios").doc(uid).collection("HorariosPresencial").doc(dia).collection(dia+"1").add({
         hora:horacita,
         timestamp: firebase.firestore.FieldValue.serverTimestamp()
        }).then((docRef) => {
        db.collection("Usuarios").doc(uid).collection("HorariosPresencialDoctor").doc(dia).collection(dia+"1").doc(docRef.id).set({
         hora:horacita,
         timestamp: firebase.firestore.FieldValue.serverTimestamp()
        }).then((docRef) => {
        window.location.reload();
  ...
  mostrar_presencial()
  {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Lunes/Lunes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Martes/Martes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        martes.push(doc.data().hora);
         ...
         db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Miercoles/Miercoles1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Jueves/Jueves1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Viernes/Viernes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Sabado/Sabado1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        ...
        db.collection("Usuarios/" + uid + "/HorariosPresencialDoctor/Domingo/Domingo1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        domingo.push(doc.data().hora);
            ...
            horariopresencial.innerHTML +=
              `
                ...
                <label>Lunes:</lable>${lunes.map((val,index) =>`<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Martes:</lable>${martes.map((val,index) => `<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Miercoles:</lable>${miercoles.map((val,index) => `<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Jueves:</lable>${jueves.map((val,index)=>`<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Viernes:</lable>${viernes.map((val,index) => `<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Sabado:</lable>${sabado.map((val,index) => `<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                <label>Domingo:</lable>${domingo.map((val,index) => `<a>${val}</a>&nbsp&nbsp&nbsp&nbsp`).join(" ")}</br>
                `
              });
     })
  ...
  mostrar_linea()
  {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Lunes/Lunes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        lunes.push(doc.data().hora);
        });
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Martes/Martes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        martes.push(doc.data().hora);
        });
         ...
         db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Miercoles/Miercoles1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        miercoles.push(doc.data().hora);
        });
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Jueves/Jueves1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        jueves.push(doc.data().hora);
        });
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Viernes/Viernes1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        viernes.push(doc.data().hora);
        });
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Sabado/Sabado1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        sabado.push(doc.data().hora);
        });
        ...
        db.collection("Usuarios/" + uid + "/HorariosLineaDoctor/Domingo/Domingo1").orderBy("timestamp","asc").get().then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
        console.log(`${doc.id} => ${doc.data()}`);
        domingo.push(doc.data().hora);
        });
    ...
    return (
      <div onload={this.inicio}>
        <div className="App" id="datos">
          <a href="/principaldoctor">Citas</a><br />
          <a href="" onClick={this.cerrar_sesion}>Cerrar Sesion</a><br />
          <h1>Perfil Doctor</h1>
          <img src="" style={{ width:10+'%',height:150+'px'}} id="imagenperfil"/><br/>
          <label>Nombre:Dr.</label><label id="nombre"></label><br />
          <label>Edad:</label>  <label id="edadMedico"></label><br />
          <label>Correo:</label> <label id="correo"></label><br />
          <label>Cedula:</label> <label id="cedula"></label><br />
          <label>Costo de la consulta:</label> <label id="consulta"></label><label>$</label><br />
          <label>Descripsion:</label> <label id="descripcion"></label><br />
          <label>Telefono:</label><label id="telefono"></label><br />
          <label>Estado:</label><label id="estado"></label><br />
          <label>Especialidad:</label><label id="especialidad"></label><br />
          <label>Termino de inscripcion:</label>  <label id="terminoinscripcion"></label><br />
        ...
        <div style={{display:'none'}} id="modificar">
          ...
          <label>Costo de la consulta</label><br/><input type="number" id="costomodificar"></input><br/>
          <label>Telefono</label><br/><input type="text" id="telefonomodificar"></input><br/>
          <label>Descripcion</label><br/><input type="text" id="descripcionmodificar"></input><br/>
          <input type="button" value="Modificar" onClick={this.editar1}></input>
        </div>
        <div id="pago">
        <h2>Pagar inscripcion</h2>
        <a href="https://buy.stripe.com/test_6oE17fdYl8pQ2c05kk">Pago Mensual</a><br/>
        </div>
        ...
        <input type="button" value="Agregar" onClick={this.agregar_horario}></input>
        </div><br/>
